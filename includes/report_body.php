...
// ---------------- all transactions (one combined statement + PDF) ----------
// Every kind of transaction in the chosen period as ONE chronological
// statement: sales bills, purchase bills, payments in/out, expenses and
// returns - with tick-boxes deciding which record types go in, so the same
// screen (and its PDF export) can be a full statement or e.g. just
// "sales + payments in" for a month.
if ($r === 'all_txn') {
    $typesAll = [
        'sale' => '🧾 Sales Bills', 'purchase' => '📦 Purchase Bills',
        'payin' => '💰 Payment In', 'payout' => '💸 Payment Out',
        'expense' => '🧾 Expenses',
        'sreturn' => '↩️ Sales Return', 'preturn' => '↪️ Purchase Return',
    ];
    if (!can('expenses.view')) unset($typesAll['expense']);
    $selTypes = array_values(array_intersect(explode(',', get('types') !== '' ? get('types') : implode(',', array_keys($typesAll))), array_keys($typesAll)));
    if (!$selTypes) $selTypes = array_keys($typesAll);
    ...
    if (in_array('payin', $selTypes, true)) {
        foreach (all("SELECT py.pay_date d, py.created_at ct, py.amount amt, py.mode, py.ref_type, py.ref_id, COALESCE(p.name,'Walk-in') who, py.notes FROM payments py LEFT JOIN parties p ON p.id = py.party_id WHERE py.direction = 'in' AND ' . money_cash_only_sql('py.mode') . ' AND py.pay_date BETWEEN ? AND ?", [$from, $to]) as $x)
    ...
    if (in_array('payout', $selTypes, true)) {
        foreach (all("SELECT py.pay_date d, py.created_at ct, py.amount amt, py.mode, py.ref_type, COALESCE(p.name,'-') who, py.notes FROM payments py LEFT JOIN parties p ON p.id = py.party_id WHERE py.direction = 'out' AND ' . money_cash_only_sql('py.mode') . ' AND py.pay_date BETWEEN ? AND ?", [$from, $to]) as $x)
...
// One-click integrity audit of the live data: every check is a read-only
// query for a situation that should NEVER happen (cash carrying a bank id,
// a serial sold twice, bill totals that don't add up...). Green = clean.
// Built so the owner can self-diagnose the moment "કંઈક ખોટું લાગે છે".
if ($r === 'health' && is_full_admin()) {
...
if ($r === 'branch_staff' && can('reports.profit')) {
    ...
    $staffRows = all("SELECT u2.name, COUNT(s.id) bills, COALESCE(SUM(s.total),0) revenue,
                       ...
                       COALESCE((SELECT SUM(amount) FROM payments WHERE created_by = u2.id AND direction = 'in' AND ' . money_cash_only_sql() . ' AND pay_date BETWEEN ? AND ?),0) collected
...
if ($r === 'cashbook') {
    ...
    foreach (all("SELECT pay_date d, SUM(amount) a, mode, (party_id IS NULL) walkin FROM payments WHERE direction='in' AND ' . money_cash_only_sql() . ' AND pay_date BETWEEN ? AND ? GROUP BY pay_date, mode, (party_id IS NULL)", [$from, $to]) as $x)
        $in[] = ['d' => $x['d'], 'desc' => ($x['walkin'] ? 'Walk-in collection (' : 'Party receipt (') . $x['mode'] . ')', 'in' => $x['a'], 'out' => 0];
    foreach (all("SELECT pay_date d, SUM(amount) a, mode, (ref_type = 'sales_return') refund FROM payments WHERE direction='out' AND ' . money_cash_only_sql() . ' AND pay_date BETWEEN ? AND ? GROUP BY pay_date, mode, (ref_type = 'sales_return')", [$from, $to]) as $x)
    ...
    $openingCb = (float)val("SELECT COALESCE(SUM(CASE WHEN direction='in' THEN amount ELSE -amount END),0)
                             FROM payments WHERE ' . money_cash_only_sql() . ' AND pay_date < ?", [$from]);
