...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && post('do') === 'save') {
    ...
    if ($amt > 0) {
        q('INSERT INTO expenses (exp_date, category, amount, mode, notes, location_id, created_by) VALUES (?,?,?,?,?,?,?)',
          [post('exp_date', today()), post('category', 'General'), $amt, post('mode', 'cash'), post('notes'), $u['location_id'], $u['id']]);
