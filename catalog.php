...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && post('do') === 'order') {
    ...
    if ($name && $mobile && $cart) {
        ...
        if ($clean) {
            q('INSERT INTO web_orders (customer_name, mobile, address, notes, items_json, total) VALUES (?,?,?,?,?,?)',
              [$name, $mobile, post('address'), post('order_notes'), json_encode($clean, JSON_UNESCAPED_UNICODE), $total]);
            $oid = insert_id();
...
.cat-chip { white-space: nowrap; padding: 7px 14px; border-radius: 999px; background: #fff; border: 1px solid var(--border); font-size: 13px; cursor: pointer; }
...
.cat-card { background: #fff; border-radius: 14px; overflow: hidden; box-shadow: 0 1px 4px rgba(0,0,0,.08); display: flex; flex-direction: column; }
.cat-card img { width: 100%; height: 135px; object-fit: cover; background: #f1f5f9; }
.cat-card .ph { width: 100%; height: 135px; display: flex; align-items: center; justify-content: center; font-size: 42px; background: #f1f5f9; }
...
.qtyrow button { width: 34px; height: 34px; border-radius: 8px; border: 1px solid var(--primary); background: #fff; color: var(--primary); font-size: 17px; font-weight: 700; cursor: pointer; }
...
.order-box { background: #fff; width: 100%; max-width: 520px; border-radius: 18px 18px 0 0; padding: 20px 16px; max-height: 88vh; overflow-y: auto; }
